  ...
  useEffect(() => {
    ...
    const fetchCoursesData = async () => {
      if (user && user.role === 'instructor') {
        try {
          const response = await fetch(`${BASE_URL}/courses?instructorId=${user.id}`);
          const data = await response.json();
          setCourses(data);
        } catch (error) {
